...
# Read
    ...
    @classmethod 
    def get_all_other_users(cls, id):
        data = {
            "id": id
        }
        query = """
...
# Update
    ...
    def update(cls, id):
        ...
        query = """
        UPDATE users 
        SET first_name = %(first_name)s, last_name = %(last_name)s, email = %(email)s, password = %(password)s, birthday = %(birthday)s
        WHERE id = %(id)s
        ;"""
